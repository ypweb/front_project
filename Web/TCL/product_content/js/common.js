$(function(){
  ...
  var product_brourl=window.location.href,prourl_pos=product_brourl.lastIndexOf("/")!=-1?product_brourl.lastIndexOf("/")+1:-1;
  var product_cururl=prourl_pos=-1?product_brourl.substring(prourl_pos,prourl_pos+4):"";
	/*初始化车型面板图标*/
			  ...
			  if(product_title_icon[ii].indexOf(",")!=-1){
				  ...
				  for(var kk=0;kk<temp_protitle.length;kk++){
					  if(title_text.indexOf(temp_protitle[kk])!=-1){
						 ...
						 if(product_cururl.indexOf("home")!=-1){
							 title_obj.css({"background":"url(images/product_title_icon.gif) no-repeat "+tempposx+"px -"+tempii*30+"px"});
						 }else if(product_cururl==""){
							 title_obj.css({"background":"url("+product_brourl+"/cwebresource/chelian/images/product_title_icon.gif) no-repeat "+tempposx+"px -"+tempii*30+"px"});
						 }else{
							 title_obj.css({"background":"url(../images/product_title_icon.gif) no-repeat "+tempposx+"px -"+tempii*30+"px"});
						 }
						 break; 
  ...
  /*成员网站和国际化*/
  $('.select_sim a,.select_language a').click(function(e){
    	e.preventDefault();
  ...
  $(".select_sim li,.select_language li").click(function(){
		var _text= $(this).text();
