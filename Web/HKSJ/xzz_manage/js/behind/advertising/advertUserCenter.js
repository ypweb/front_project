...
$(function() {
	tb = $('#div_table03').table({
		 		 ...
		 		 return [
					$.jsonToControl(
					 {tag:'a',style:'cursor: pointer', child:{tag:'span',child:'上架'},events:{click:function(){
						 $.post('advertUserCenter/advertUserCenterUL.do',{id:r.id,status:0},function(result){
						 		if(result==true){
					 ...
					 $.jsonToControl(
							 {tag:'a',style:'cursor: pointer', child:{tag:'span',child:'下架'},events:{click:function(){
								 $.post('advertUserCenter/advertUserCenterUL.do',{id:r.id,status:1},function(result){
								 		if(result==true){
				...
				$.jsonToControl({
					...
					events: {
						click: function() {
							...
							$("#windowIf").attr("src", 'advertUserCenter/advertUserCenterEdit.do?id=' + r.id);
							$("#windowDiv").window({
								title: '编辑个人中心广告',
								width: 400,
								height: 300
		...
		url: 'advertUserCenter/advertUserCenterList.do',
		autoload: true,
		total: 'totalCount',
		root: 'items',
		footbar: {
...
function query()
{
	tb.load({start:0,name:$('#name').val(),status:$('#status').val(),position:$("#position").val()});
}
...
function toAdd() {
	window.frames["windowIf"].document.body.innerHTML = "";
	document.getElementById("windowIf").style.display = "";
	$("#windowIf").attr("src", "advertUserCenter/advertUserCenterEdit.do");
	$("#windowDiv").window({
		title: '新增个人中心广告',
		width: 400,
		height: 300
