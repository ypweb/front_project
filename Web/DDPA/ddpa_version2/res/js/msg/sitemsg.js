...
(function($){
	...
	$(document).ready(function(e) {
		...
		function initData(){
			$.ajax({
				...
				success:function(data){
					...
					if(ftl!= false){
						$ftlbox.html(ftl);
						$ftlbox.find(".checkbox").checkbox({handler:checkSingle});
					}
				}
			})
			$.ajax({
			...
			if(isChecked){
				for(i=0; i<l;i++){
					...
					if(cb.hasClass("unchecked")){
						...
						cb.find("input").attr("checked","checked")
					}
				}
				...
				for(i=0; i<l;i++){
					...
					if(cb.hasClass("checked")){
						...
						cb.addClass("unchecked").removeClass("checked");
						cb.find("input").removeAttr("checked");
					}
				}
			}
		}
		...
		function checkSingle(e){
			var $target = $(e.target), tbody = $target.parents(".tbody");
			var unchecked = tbody.find(".checkbox.unchecked");
			var thead_checkbox = tbody.siblings(".thead").find(".checkbox");
			
			if(unchecked.size() == 0 && thead_checkbox.hasClass("unchecked")){
				thead_checkbox.attr("status","checked").addClass("checked").removeClass("unchecked").find("input").attr("checked","checked");
			}else if(unchecked.size() != 0 && thead_checkbox.hasClass("checked")){
				thead_checkbox.attr("status","unchecked").addClass("unchecked").removeClass("checked").find("input").removeAttr("checked");
			}
			...
			$.ajax({
				...
				success: function(data){
					...
					if(data=="nologin"){
						...
						return false;
					}else if(data.status == "y" || data.success=="true"){
		...
		function getDetail(obj){
			...
			var ftl = $("<div class='msgdetail'></div>"), $curtab = $(".tabc.cur"), msgtype = $curtab.attr("msgtype");
			ftl.append($(".msgdetail").html());
			$.ajax({
