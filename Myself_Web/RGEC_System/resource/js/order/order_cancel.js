...
require(['jquery','bootstrap','dialog','querydata','validform','common','submenu','themetitle','pagination','sort','checkbox','gridaction','rule','commonfn'],function($,$strap,undefined,undefined,undefined,Common,undefined,undefined,undefined,Sort,CheckBox,GridAction,Rule,CommonFn) {
	$(function() {
			...
			//html模板
			var htmlcontent='<tr><td><input value="$chkId" data-state="1" type="checkbox" name="chkId"></td><td><div class="order-thumbnail"><a href="$url"><img src="$img" ></a><a href="$url">$orderName</a></div></td><td><p>$orderNo</p></td><td>$Library</td><td>$Type</td><td>$Status</td><td>$recAddress</td><td>$recName</td><td>$recPhone</td><td><p>$orderPay</p></td><td><span data-action="$actiontype" data-value="$actionvalue">操作</span></td></tr>';
			
	
	
			//查询搜索
			$grid_querybtn.queryListData([$orderno,$recphone,$recname,$library,$status,$type],2000,function(sparam){
			...
			$orderinfo_sure.on('click',function(){
					var tobepaid_data=CheckBox.getCheckBox(),
							...
							if(len==0){
								...
								GridAction.gridHandler($orderinfo_sure,function(objs){
										//进入需要确认是否执行操作的流程
										if(objs.dialog){
										...
										if(issucc){
											tipdia.content('<span class="g-c-cyan1 g-btips-succ">取消订单成功</span>').show();
										}else{
											tipdia.content('<span class="g-c-red4 g-btips-error">取消订单失败</span>').show();
										}
								});
							}
