...
$(document).ready(function(e) {
	...
	function setBidStatus(status){
		...
		if(status == "#0"){
			...
			//满标
			statustxt.text("已满标");
			f.find("button").addClass("disabled").attr("disabled","disabled");
	...
	function calInvest(e){
		var $target = $(e.target), v = $target.val(), rate = $("#rate").attr("data-v"), months = $("#months").attr("data-v");
		v = parseFloat(v);
			...
			success:function(data){
				if(data){
					if(data.success==true){
						...
						if(data.availableAmount && parseFloat(data.availableAmount) < 1){
							location.hash="1";
						}
		...
		$(".bidform").Validform({
			...
			datatype:{
				"money":function(gets,obj,curform,regxp){
					var reg = /^[1-9]\d*\.?\d{0,2}$/;
					if(!reg.test(gets) || gets == ""){return false;}
					var usable = $("#usable").attr("data-v");
					usable = parseFloat(usable);
					if(gets>usable){
						$.blockUI({  
						title:'提示', 
						message:'<div class="warn"><p>账户余额不足，立即去<a href="../../res/fund/trade/rechargeInit">充值</a></p><a href="javascript:;" onclick="$.unblockUI()" class="closeWarn">关闭</a></div>',
						css:{
							width:'30%',
							cursor:'auto',
							top:top
	...
	function setLogin(){
		...
		if(isLogin==1){
			...
			$("#usable").siblings("a").text("登录").attr("href","../account/user/login");
		}
	}
	...
	var noreply = '<form name="answer" class="answerform">'
                ...
                + '</form>';
	var replay = '<form name="answer" action="../../res/js/question/add" class="answerform">'
               + '<div><label>答复：</label>'
