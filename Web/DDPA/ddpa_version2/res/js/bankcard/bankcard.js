...
$(document).ready(function(e) {
	...
	function init(){
		...
		$("#cardNo").focus(function(){
			...
			$(this).after("<div class='Validform_checktip float'>仅支持借记卡</div>");
		});
		$("#cardNo").focusout(function(){
			$(this).siblings(".float").replaceWith("");
		...
		//修改银行卡号
		//$("#e_cardnum").blur(isExistBankCard);
	}
		...
		$.ajax({
			...
			success:function(data){
				if(data.status == "y"){
					...
					$target.siblings(".Validform_checktip").removeClass("Validform_wrong").addClass("Validform_right").text(data.info);
				}else{
					$target.addClass("Validform_error")
					$target.siblings(".Validform_checktip").addClass("Validform_wrong").removeClass("Validform_right").text(data.info);
				}
			}
		})
	}
	...
	function addcard(card){
		...
		$("#cardform","#addform").find("button").removeAttr("disabled");
		$.blockUI({ 
				message:  $("#addform"),
				css:{
					top:"20%",
		...
		$.ajax({
			...
			success: function(data){
				if(data.status=="y"){
					...
					//填充银行卡信息
					cardform.find("button").removeAttr("disabled");
					cardform.siblings("input[name='def']").val(data.bankCard.isDefault);
					...
					cardform.find("#e_cardnum").val(formatBCNo(data.bankCard.cardNo)).attr("ajaxurl","isExistBankCard?oldNo="+data.bankCard.cardNo);
					cardform.find("#e_oldNo").val(data.bankCard.cardNo);
	...
	cardforms = $(".cardform").Validform({
		...
		beforeSubmit:function(curform){
			...
			var has = curform.siblings("input[name='def']").val();
			//console.log(has)
			curform.find("button").attr("disabled","true");
			if($("#defaultCard").size()!=0 && ($defcard.attr("status")=="checked" && has=="0")){
				...
				if(confirm("当前默认提现银行卡为"+bankN+"尾号"+last4No+"的卡号，是否确认更换？")){
					...
					$defcard.click();
					curform.find("button").removeAttr("disabled");
					return false;
				};
			}
	...
	//检测银行卡号
	function checkEditNo(noInput){
		var oldNo = noInput.attr("data-old"), 
			newNo = noInput.val();
		oldNo = oldNo.replace(/(\s*)/g,""),
		newNo = newNo.replace(/(\s*)/g,"");
		if(oldNo == newNo){
			noInput.addClass("Validform_error").siblings(".Validform_checktip").addClass("Validform_wrong").removeClass("Validform_right").text("银行卡存在");
			return false;
		}else{
			noInput.removeClass("Validform_error").siblings(".Validform_checktip").removeClass("Validform_wrong").text("");
			return true;
		}
	}
	...
	function submitForm(f){
		...
		data.oldNo = f.find("input[name='oldNo']").val();
