	...
	$.ajax({
		...
		success:function(data){
			...
			if(data.listProduct.length>0){
				...
				if(o_typelistwrap.next("p"))o_typelistwrap.next("p").detach();
				o_typelistwrap.html(pro_typestr);
			}else{				
				$("<p style=\"float:left;width:200px;height:20px;line-height:20px;font-family:'宋体';margin-left:20px;overflow:hidden;color:#666;\">暂无相应车型</p>").insertAfter(o_typelistwrap);
			}
		}
	});
}
