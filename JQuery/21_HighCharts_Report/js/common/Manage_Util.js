...
/*认证检测*/
/*../../borrow/user/checkedDetail*/
function verif_Infos(){
	var verif_obj={},asurl="",tips="";
	$.ajax({
		url:"../../borrow/user/checkedDetail",
		type:"post",
		data:{},
		async:false,
...
/*检测提现金额*/
function testTradeAmount(strs){
	  var mtext=/^(([1-9]{1}\d{0,5})|0)((\.{0}(\d){0})|(\.{1}(\d){2}))$/
	  var values=strs.replace(/(\s*)/g,"");
	  if(mtext.test(values)){
		  var tempvalue=parseInt(values);
		  if(tempvalue<100){
			  return "提现金额必须大于等于100";
		  }else if(tempvalue>500000){
			  return "提现金额不超过50万元";
		  }else{
			  return true;
		  }
