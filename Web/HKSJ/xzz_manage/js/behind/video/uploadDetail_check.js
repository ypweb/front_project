...
$(function() {
	...
	$("#bbjx").html("("+ parent.$("#CHECK_VIDEO_MIN").val()+"~"+ parent.$("#CHECK_VIDEO_MAX").val()+",审核不通过或多次通过可不填)");
...
function pass(type)
{
	...
	if(type==3) //通过
	{
		if(r.ADMIN_OR_USER==2 && (r.VIDEO_BB==null || r.VIDEO_BB=='') && $("#HFTYPE").val()=="1" ) //首次上传,或多次通过，后台上传,不是惠粉 没有播币	
		{
			...
			if(parseInt(bb)>parseInt(parent.$("#CHECK_VIDEO_MAX").val()) || parseInt(bb)<parseInt(parent.$("#CHECK_VIDEO_MIN").val()))
			{
	...
	{
		if($("#APPROVE_CONTENT").val().replace(/[ ]/g,"")=="")
		{
			$.messager.alert("提示","审核内容不能为空","info",function(){document.getElementById("APPROVE_CONTENT").focus();});
			return;
		}
		if($("#APPROVE_CONTENT").val().length>128)
		{
			$.messager.alert("提示","审核内容过长,最大长度为128","info",function(){document.getElementById("APPROVE_CONTENT").focus();});
			return;
		}
	}
