...
$(function(){
	...
	products_brand.find("li").click(function(){
		...
		/*判断车型系列是否被选中*/
		products_brandlist.find("li").each(function(){
			var pbrandlist_obj=$(this);
			if(typeof pbrandlist_obj.attr("class")=="string"&&pbrandlist_obj.attr("class")!=""){
				pbrandlist_tempstr=pbrandlist_obj.attr("id");
				return false;
			}
        });
		if(pbrandlist_tempstr!=""){
			handle_GetDevType(contextpath,[cur_brandattr,pbrandlist_tempstr],products_typelist);
			products_names.val()!=""?proconsel_list.splice(0,4,cur_brandattr,products_names.val(),pbrandlist_tempstr,""):proconsel_list.splice(0,4,cur_brandattr,"",pbrandlist_tempstr,"");
		}else{
			products_names.val()!=""?proconsel_list.splice(0,4,cur_brandattr,products_names.val(),"",""):proconsel_list.splice(0,4,cur_brandattr,"","","");
		}
		handle_AjaxReq(proconsel_list,products_resultshow,contextpath,currentPage);
	});
	...
	products_brandlist.find("li").click(function(){
		var pblist_obj=$(this),pblist_attr=pblist_obj.attr("id"),pbrand_tempstr="";
		pblist_obj.addClass("pro_brandlistsel").siblings().removeClass("pro_brandlistsel");
		/*判断品牌是否被选中*/
		products_brand.find("li").each(function(){
			var pbrand_obj=$(this);
			if(typeof pbrand_obj.attr("class")=="string"&&pbrand_obj.attr("class")!=""){
				pbrand_tempstr=pbrand_obj.attr("id");
				return false;
			}
        });
		products_names.val()!=""?proconsel_list.splice(1,3,products_names.val(),pblist_attr,""):proconsel_list.splice(1,3,"",pblist_attr,"");
		pbrand_tempstr==""?handle_GetDevType(contextpath,[pblist_attr],products_typelist):handle_GetDevType(contextpath,[pbrand_tempstr,pblist_attr],products_typelist);
		handle_AjaxReq(proconsel_list,products_resultshow,contextpath,currentPage);
	});
	...
	product_clears.click(function(){
		...
		products_typelist.html("");
		$("#producttype_null").html("暂无相关产品");
		handle_AjaxReq(proconsel_list,products_resultshow,contextpath,1);
	});
...
function handle_GetDevType(o_url,o_data,o_typelistwrap){
	var temp_data="";
	o_data.length==2?temp_data="series="+o_data[0]+"&devtype="+o_data[1]+"&flag=1":temp_data="devtype="+o_data[0]+"&flag=1";
	$.ajax({
		type:"post",
...
/*
数据请求和数据处理展现
参数说明:放置查询条件的数组,放置结果集容器,请求地址,分页参数
*/
function handle_AjaxReq(){
	...
	$.ajax({
		...
		success:function(data){
			...
			if(data.listProduct.length>0){
				$.each(data.listProduct,function(index,item){
						pro_resultstr+="<li><div class=\"pro_resultcell\"><a class=\"pro_resultimg\" target='_blank' href=\""+data.url+""+item.seqid+".html\"><img alt=\"\" src=\""+data.imgpath+""+item.imgurl+"\"/></a><a class=\"pro_resulttips\" target='_blank' href=\""+data.url+""+item.seqid+".html\">"+item.devname+"</a></div></li>"
					});
				o_result.html("<ul>"+pro_resultstr+"</ul>");
			}else{
				o_result.html("<p style=\"padding:10px 0;text-align:center;width:898px;margin:0 auto;border-bottom:1px solid #ddd;border-left:1px solid #ddd;border-right:1px solid #ddd;\">暂无符合条件的产品</p>");
			}
