...
define(['jquery'],function($){
		...
		return {
			 ...
			 *
			 * */
			getFromDraft:function(){

				//获取文章id和用户id，判断是否存在文章id
				this.getUserInfo();

				//to do
				var self=this,
					articleObj=this.articleinfo;

				if(articleObj.articleid!==null&&articleObj.userid!=null){
					//发送请求获取结果集
					$.ajax({
						url:articleObj.url,
						data:{
						...
						success:function(res){
							...
							if(len===0||list===undefined){
								//如果为非则重置值
								articleObj.userid=null;
								articleObj.articleid=null;
								articleObj.url=null;
							}else{
								var datas=list[articleObj.articleid],
									content=datas['content'],
									auth=datas['auth'],
									tag=datas['tag'];

								for(var i= 0,sublen=content.length;i<sublen;i++){
									//反向解析标签并生成对应html标签
			...
			getUserInfo:function(info){
				...
				if(info){
					...
					this.articleinfo={
						userid:1,
						articleid:parseInt(Math.random()*10,10),
