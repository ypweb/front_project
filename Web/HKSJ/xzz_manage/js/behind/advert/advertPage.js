var select;
	$(function() {
		...
		$.ajax({
			...
			success : function(result) {
				var listHtml = "<option value=''>默认全部</option><option value='0'>首页</option><option value='1'>专辑</option>";
				...
				.table(
						{
							...
							cm : [
									{
										dataindex : 'PAGE_NAME', // 显示数据在行中的属性
										headtext : '名称'
									},
									{
									...
									{
										dataindex : 'PIC_INDEX', // 显示数据在行中的属性
										headtext : '排序号'
									},
									{
										dataindex : 'PAGE_STATUS', // 显示数据在行中的属性
										headtext : '状态',
										render : function(i, j, c, r) {
											return '1' == c ? '启用' : '禁用';
										}
									},
									{
										...
										headtext : '操作',
										render : function(i, j, c, r) {
											var isDisabled = r.PAGE_STATUS == "1" ? "禁用"
													: "启用";
											var disabledVal = r.PAGE_STATUS == "1" ? "0"
													: "1";
											return [
													$.jsonToControl({
																...
																events : {
																	click : function() {
																		...
																		$("#windowIf").attr("src",'toUpdateAdvertPage.do');
																		$("#windowDiv").window({title : '编辑页面专题:'+r.PAGE_NAME,width:530,height:300}).window('open');
																		return false;
																	}
																}
															}),
															...
															.jsonToControl({
																...
																events : {
																	click : function() {
																		$
																				.post(
																						'updateAdvertPageStatus.do',
																						{
																							ID : r.ID,
																							PAGE_STATUS : disabledVal
																						},
																						function(
																								result) {
																							if (result == 1) //修改成功
																...
																events : {
																	click : function() {
																		 $.messager.confirm("确认","确定要删除?",function(ha){
																			 if(ha)
																			 {
																				 $.post('deleteAdvertPage.do',{ID:r.ID},function(result){
																				 		if(result==1)
																				 		{
																				 			tb.load();
	...
	function query()
	{	
		tb.load({start:0,TYPEONE_ID:$('#selectTypeOne').val(),PAGE_NAME:$("#advertName").val()});
	}
	...
	function toAdd()
	{
		window.frames["windowIf"].document.body.innerHTML = "";
		document.getElementById("windowIf").style.display="";
		$("#windowIf").attr("src","toAddAdvertPage.do");
		$("#windowDiv").window({title : '新增首页推荐',width:530,height:350}).window('open');
	}
