(function($){
	$(function(){
		...
		var mr_selectjjk=$("#mr_selectjjk"),mr_selectxyk=$("#mr_selectxyk");
		/*隐藏域:已选，最新选择、选择重复提示区*/
			   ...
			   mr_alreadywrap.hide();
					...
					if(ishaveli==0){
						...
						mr_bankwrap.find("li").each(function(index, element) {
                            var curobj=$(element),curtext=curobj.not("span").text();
						...
						if(mr_bankwrap.find("li").size()==0){
							...
							$(tempstr).insertBefore(mr_bankwrap.find("li").each(function(index, element){
								var curobj=$(element),curtext=curobj.not("span").text(),spanvalue=curobj.find("span").attr("class").slice(3,-4);
								bankres.push([curtext,spanvalue]);
								curobj.find("div").addClass(aobj[tempvalue].pclass);
							}).end().find("li:first"));
						}
					}
					bankres.unshift([tempvalue,temptype]);
			}
			/*大写初始化*/
		...
		/*绑定更多银行选择区域鼠标事件*/
		mr_selectwrap.hover(function(){},function(e){
			$(this).hide();
			mr_alreadywrap.show();
		});
		/*绑定选择不同类型*/
		...
		$("#mr_selectjjk,#mr_selectxyk,#mr_selectdsf").live("click",function(e){
			...
			var curobj=$(e.target),curtext,selstr="",banklen=bankres.length,parobj="",wrapid="";
			...
			for(var i=0;i<banklen;i++){
				if(curtext==bankres[i][0]&&wrapid==bankres[i][1]){
					bankflag=true;
					mr_bankseltips.html("您已经选择此支付方式,请重新选择");
			...
			if(!bankflag){
				/*默认列表中不存在新选择的银行*/
				/*队头填数据*/
				bankres.unshift([curtext,wrapid]);
				if(bankres.length>=3){
					/*队尾删数据*/
					bankres.length=3;
				}
		...
		mr_bankwrap.click(function(e){
			...
			var curobj=$(e.target),curtext="";
			if(tempnode=="p"||tempnode=="div"||tempnode=="span"){
				var parobj=curobj.parent();
				curtext=parobj.not("span").text();
				parobj.addClass("mr_banksel").siblings().removeClass("mr_banksel");
			}else if(tempnode=="li"){
				curtext=curobj.not("span").text();
				curobj.addClass("mr_banksel").siblings().removeClass("mr_banksel");
			}
			beforebank.val(curtext);
			thirdPayType.val(curtext);
		});
		/*充值表单校验*/
		$("#mr_reload").validate({
				rules : {
