...
$(function(){
	...
	var init_brandlist;
	if(init_index==''){
		init_brandlist =products_brandlist.find("li:eq("+init_index+")").attr("id");
	}else{
	...
	products_brand.find("li").click(function(){
		...
		$.ajax({
			...
			success:function(data){
				...
				var brand_typestr="<table><tr><td width='35%'><img class='products_brandimg' alt='' src='../../../../../ResponsiveWebDesign_Web/TCL_carweb/source/js/product/"+data.imgpath+""+data.webCarBrand.brandimg+"'/></td>" +
...
function handle_AjaxReq(){
	...
	$.ajax({
		...
		success:function(data){
			...
			if(data.listProduct.length>0){
				$.each(data.listProduct,function(index,item){
						pro_resultstr+="<li><a class=\"pro_resultimg\" target='_blank' href=../../../../../ResponsiveWebDesign_Web/TCL_carweb/source/js/product/""+data.url+""+item.seqid+".html/"><img alt=\"\" src=../../../../../ResponsiveWebDesign_Web/TCL_carweb/source/js/product/""+data.imgpath+""+item.imgurl+"/"/></a><a class=\"pro_resulttips\" target='_blank' href=../../../../../ResponsiveWebDesign_Web/TCL_carweb/source/js/product/""+data.url+""+item.seqid+".html/">"+item.devname+"</a></li>"
					});
