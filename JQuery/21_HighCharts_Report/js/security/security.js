...
$(document).ready(function(e) {
	...
	function getAcbyOP(obj){
		...
		var param={};
	       ...
	       $.post("../../sys/sms/sendUserSms",param,function(data){
	    	   if(data.success){
	               obj.hide();
	               obj.siblings("#after_getac").replaceWith("");
		   			obj.after("<span id='after_getac'><span id='ac_timer1' class='timer'>60</span>秒有效</span><span id='t30' class='authtip'>验证码已发送至您的手机,请注意查收</span>");
	...
	function checkACByPassword(obj){
		...
		//验证发送的验证码
		$.ajax({
			url:"updatePpproveMobilePhoneForPhone",
			type:"POST",
	...
	function getAuthCode(obj){
	       ...
	       $.post("../../sys/sms/sendUserSms",param,function(data){
	            if(data.success){
	               obj.hide();
	               obj.siblings("#after_getac").replaceWith("");
		   			obj.after("<span id='after_getac'><span id='ac_timer2' class='timer'>60</span>秒有效</span><span id='t30' class='authtip'>验证码已发送至您的手机,请注意查收</span>");
	...
	var forms = $(".form").Validform({
		...
		datatype:{
			...
			"m_f":function(gets,obj,curform,regxp){
				gets = gets.replace(/(\s*)/g,"");
				var reg = /^(13[0-9]|15[012356789]|18[01236789]|14[57])\s?[0-9]{4}\s?[0-9]{4}$/;
				if(!reg.test(gets)){
					return false;
				}
			},
			"ischi_Email":/^([0-9A-Za-z\-_\.]+)@([0-9a-z]+\.[a-z]{2,3}(\.[a-z]{2})?)$/
		},
			...
			success:function(data){
				...
				if(data.status=="y"){
					...
					if(data.info.indexOf("@")<0){ //如果不是邮箱
						...
						setTimeout(function(){ timer=null;  location.hash = ""; window.location.reload();},3800);
					}else{
						location.hash = "";
