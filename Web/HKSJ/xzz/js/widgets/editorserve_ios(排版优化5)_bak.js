											...
											if(self.secectindex==-1){
												...
												(function(){
													...
													if(data!=undefined&&data['type']=='txt'){
														...
														data['value']=txt + anode + '<a data-id="'+self.secectindex+'" href="'+value+'" >'+url+'</a>';
...
//添加高亮效果并识别是否是同一类格式化
			...
			* */
			...
			cancelPublish:function(){
				var self=this;
				if((self.stepdata.length!=0&&self.index!=-1)||!this.isRequire(this.$article_title)){
					self.callback=function(){
						self.saveDraft();
					};
					...
					if(type=='in'){
						...
						(function(){
							...
							if(txt==''){
								...
								self.index=0;
								if(self.stepdata.length==0||self.index==-1){
									tempstep['value']=txt;
									tempstep['type']='txt';
									tempstep['id']=0;
									tempstep['style']='';
						...
						if(prevflag!=undefined){
							...
							(function(){
								...
								if(data!=undefined){
									...
									if(type=='txt'){
										//如果前一个元素是文本元素则继续追加
										self.stepdata[self.secectindex]['value']=self.$article_content.find("p[data-id='"+self.secectindex+"']").html();
									}
								}else{
									...
									if(type=='txt'){
										//如果前一个元素是文本元素则继续追加
										self.stepdata[self.index]['value']=self.$article_content.find("p[data-id='"+self.index+"']").html();
									}
								}
								self.htmlShow();
							}());
