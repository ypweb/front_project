...
define(['jquery','area'],function ($,Area) {
	...
	this.areaInit=function(set){
			...
			//初始化数据
			var citys=[set.$province,set.$city,set.$area],
							i=0,
							len=citys.length;
					for(i;i<len;i++){
							var $current=citys[i];
							if($current!=null){
									...
									if(selector.indexOf('province')!==-1){
											data=Area[this.province_key];
											var $pinput=$current.next('input')||$current.prev('input');
											if($pinput.length!=0){
													var ptext=$pinput.val();
													ptext!=''?this.initData('province',$current,ptext):this.initData('province',$current);
											}else{
												this.initData('province',$current);
											}
											//绑定选择事件
											$current.on('change',function(){
													...
													value=$this.val(),
													key='',
													$option=$this.find('option').each(function(){
															var $item=$(this),
															isselect=$item.prop('selected');
															if(isselect){
																	key=$item.attr('data-value');
																	self.city_key=self.province_key+','+key;
																	if(self.city_key!=''){
																		self.initData('city',set.$city);
																		if(self.area_key!=''){
																			self.initData('area',set.$area);
																		}
																	}
																	return false;
															}
											...
											var $cinput=$current.next('input')||$current.prev('input');
											if($cinput.length!=0){
													var ctext=$cinput.val();
													ctext!=''?this.initData('city',$current,ctext):this.initData('city',$current);
											}else{
												this.initData('city',$current);
											}
											//绑定选择事件
											$current.on('change',function(){
													var $this =$(this),
													...
													$option=$this.find('option').each(function(){
															...
															if(isselect){
																	key=$item.attr('data-value');
																	self.area_key=self.city_key+','+key;
																	if(self.area_key!=''){
																		self.initData('area',set.$area);
																	}
																	return false;
											...
											var $ainput=$current.next('input')||$current.prev('input');
											if($ainput.length!=0){
													var atext=$ainput.val();
													atext!=''?this.initData('area',$current,atext):this.initData('area',$current);
											}else{
												this.initData('area',$current);
											}
									}
							}
					}
	...
	//初始化数据
	this.initData=function(type,wrap,text){
			var data;
			if(type=='province'){
					data=Area[this.province_key];
			}else if(type=='city'){
					data=Area[this.city_key];
			}else if(type=='area'){
					data=Area[this.area_key];
			}
			var str='',
			count=0;
			if(text&&text!=''){
					for(var i in data){
							if(text==data[i]){
								str+='<option selected="true" data-value="'+i+'" value="'+data[i]+'">'+data[i]+'</option>';
								if(type=='province'){
									this.city_key=this.province_key+','+i;
								}else if(type=='city'){
									this.area_key=this.city_key+','+i;
								}
							}else{
								str+='<option data-value="'+i+'" value="'+data[i]+'">'+data[i]+'</option>';
							}
					}	
			}else{
					for(var i in data){
