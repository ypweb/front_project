(function($,w){
		...
		var impression={
				...
				init:function(wrap){
						...
						this.template='<li class="impress-item" style="$style"><div style="$linestyle"><img alt="" src="$imgsrc" ></div><p>$content</p></li>';
						this.imgsrc='../../theme/block/images/web_impressionicon.png';
						this.changeData(wrap);
				},
				/*随机生成坐标*/
				randomPos:function(){
						var self=this;
						var styleitem,temppos=0,divstyle;
						this.items.each(function(index,item){
								styleitem=item.style;
								divstyle=item.children[0].style;
								self.mathPos(index);
								styleitem.left=self.stylepos['left']+'%';
								styleitem.top=self.stylepos['top']+'px';
								divstyle.height=self.stylepos['height']+'px';
								divstyle.top='-'+self.stylepos['height']+'px';
						});
				},
				/*坐标计算*/
				mathPos:function(index){
						...
						this.stylepos['top']=index==0?20:index*70;
						this.stylepos['height']=this.stylepos['top']+85;
				},
				/*更新自身数据*/
				changeData:function(wrap){
						this.items=wrap.find('li');
						this.size=this.items.size();
						var wrapheight=this.size*80;
				...
				commentResult:function(str,res,params){
						if(str=='error'){
								...
								template=template.replace('$style','left:'+this.stylepos['left']+'%;top:'+this.stylepos['top']+'px;').replace('$linestyle','height:'+this.stylepos['height']+'px;top:-'+this.stylepos['height']+'px;').replace('$content',content);
		...
		$(function(){
				//页面元素引用
				var $impression=$('#impression_wrap'),
