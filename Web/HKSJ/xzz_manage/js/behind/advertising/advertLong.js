...
$(function() {
	tb = $('#div_table03').table({
		 		 ...
		 		 return [
					$.jsonToControl(
					 {tag:'a',style:'cursor: pointer', child:{tag:'span',child:'上架'},events:{click:function(){
						 $.post('advertLong/longArticleUL.do',{id:r.id,status:0},function(result){
						 		if(result==true){
					 ...
					 $.jsonToControl(
							 {tag:'a',style:'cursor: pointer', child:{tag:'span',child:'下架'},events:{click:function(){
								 $.post('advertLong/longArticleUL.do',{id:r.id,status:1},function(result){
								 		if(result==true){
				...
				$.jsonToControl({
					...
					events: {
						click: function() {
							...
							$("#windowIf").attr("src", 'advertLong/advertLongEdit.do?id=' + r.id);
							$("#windowDiv").window({
								title: '编辑长文广告',
								width: 650,
								height: 670
		...
		url: 'advertLong/advertLongList.do',
		autoload: true,
		total: 'totalCount',
		root: 'items',
		footbar: {
...
function query()
{
	tb.load({start:0,title:$('#title').val(),status:$('#status').val()});
}
...
function toAdd() {
	window.frames["windowIf"].document.body.innerHTML = "";
	document.getElementById("windowIf").style.display = "";
	$("#windowIf").attr("src", "advertLong/advertLongEdit.do");
	$("#windowDiv").window({
		title: '新增长文广告',
		width: 650,
		height: 670
