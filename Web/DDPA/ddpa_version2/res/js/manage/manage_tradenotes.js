		...
		$("#tn_dateinterval,#tn_sctag").click(function(e){
			if(e.target.nodeName=="LI"){
				...
				if(e.target.parentNode.id=="tn_dateinterval"){
					var tn_tempdates=diffDate(curobj.text());
...
function tn_ToDateFormat(strs){
	...
	if(units=="天"){
		if(nums>=curdays){
			...
			if(nums==curdays){
				formatd=parseInt(is_LeapYear(curyear,formatm)+curdays)-nums;
			}else if(nums>curdays){
...
//算时间差
function diffDate(strs){
	var date = DateFormat(new Date()), arrayDate = ["",date];
	
	if(strs=="最近7天"){
		arrayDate[0] = SubDays(date,7);
	}else if(strs == "1个月内"){
		var cur  = new Date(), curDay = cur.getDate(), maxDay = getMaxDay();
		if(curDay == maxDay){
			arrayDate[0] = SubDays(date,maxDay-1);
		}else{
			arrayDate[0] = DateFormat(getLastMonthToDay());
		}
	}else if(strs == "3个月内"){
		arrayDate[0] = SubDays(date,60);
	}else if(strs=="12个月内"){
		arrayDate[0] = SubDays(date,365);
	}else{
		arrayDate[0] = SubDays(date,0);
	}
	return arrayDate;
}
		...
		success: function(datas){
			...
			if(modelen==0){
				...
				for(var k=0;k<modelen;k++){
					...
					if(k==modelen-1){
						tempobj.income = tempobj.income == null? "":tradeRMBFormat("",tempobj.income);
						tempobj.expenses = tempobj.expenses == null? "":tradeRMBFormat("",-tempobj.expenses);
						tn_resdatas+="<tr class=\"tn_rdf\"><td class=\"tn_rtd1\">"+tempobj.tradeTime+"</td><td class=\"tn_rtd2\">"+tempobj.tradeType+"</td><td class=\"tn_rtd3\">"+tempobj.income+"</td><td class=\"tn_rtd4\">"+tempobj.expenses+"</td><td class=\"tn_rtd5\">"+tradeRMBFormat("",tempobj.tradeBalance)+"</td><td class=\"tn_rtd5 tn_rtdlast\">"+tempobj.remark+"</td></tr>";
					}else{
						tempobj.income = tempobj.income == null? "":tradeRMBFormat("",tempobj.income);
						tempobj.expenses = tempobj.expenses == null? "":tradeRMBFormat("",-tempobj.expenses);
						tn_resdatas+="<tr class=\"tn_rdb\"><td class=\"tn_rtd1\">"+tempobj.tradeTime+"</td><td class=\"tn_rtd2\">"+tempobj.tradeType+"</td><td class=\"tn_rtd3\">"+tempobj.income+"</td><td class=\"tn_rtd4\">"+tempobj.expenses+"</td><td class=\"tn_rtd5\">"+tradeRMBFormat("",tempobj.tradeBalance)+"</td><td class=\"tn_rtd5 tn_rtdlast\">"+tempobj.remark+"</td></tr>";
					}
				}
