...
function init (isLoadXMLSuccess : Boolean) : Boolean
{
	...
	{
		xmlArray [i] = {
			img : "",bytesLoaded : 0, bytesTotal : 0, loadComplete : false, btnMC : null, showMC : null
		}
...
function showEffect (indexID : Number) : Void
{
	...
	switch (randomNum)
	{
		...
		{
			type : Blinds, direction : Transition.IN, duration : 5, easing : None.easeNone, numStrips : 10, dimension : 0
		})
		break;
		case 1 :
		TransitionManager.start (xmlArray [indexID].showMC,
		{
			type : Fade, direction : Transition.IN, duration : 5, easing : None.easeNone
		})
		break;
		case 2 :
		TransitionManager.start (xmlArray [indexID].showMC,
		{
			type : Iris, direction : Transition.IN, duration : 5, easing : Strong.easeOut, startPoint : 5, shape : Iris.CIRCLE
		})
		break;
		case 3 :
		TransitionManager.start (xmlArray [indexID].showMC,
		{
			type : Photo, direction : Transition.IN, duration : 5, easing : None.easeNone
		})
		break;
		case 4 :
		TransitionManager.start (xmlArray [indexID].showMC,
		{
			type : PixelDissolve, direction : Transition.IN, duration : 5, easing : None.easeNone, xSections : 10, ySections : 10
		})
		break;
		case 5 :
		TransitionManager.start (xmlArray [indexID].showMC,
		{
			type : Wipe, direction : Transition.IN, duration : 5, easing : None.easeNone, startPoint : 1
		});
		break;
		default :
		TransitionManager.start (xmlArray [indexID].showMC,
