...
function update()
{
	...
	$.post('updateAdvert.do',$("#advertForm").serialize(),function(result){
		if(result==1) //修改成功
		{
	...
	window.onload = function () {
		...
		$('#container1').on(
			...
			$("#ID").val(r.ID);
			$("#ADVERT_ID").val(r.ADVERT_ID);
			$("#ADVERT_NAME").val(r.ADVERT_NAME);
			$("#ADVERT_WEIGHT").val(r.ADVERT_WEIGHT);
			$("#JUMP_URL").val(r.JUMP_URL);
			$("#ADVERT_PRICE").val(r.ADVERT_PRICE);
			$("#ADVERT_MONEY").val(r.ADVERT_MONEY);
			$("#ADVERTUSER_ID").val(r.ADVERTUSER_ID);
			$("#ADVERTUSER_ID1").val(r.ADVERT_COMPANY);
			//初始化二级类型
			$.ajax({
				type : "post",
				dataType : "json",
				...
				success : function(result) {	
					...
					$.each(result, function(i, n) {
						if((r.TYPETWO_ID).indexOf(n.TYPETWO_ID)==-1)
							listHtml += "<option value="+n.TYPETWO_ID+">"+n.TYPETWO_NAME+"</option>";
						else
							listHtml += "<option value="+n.TYPETWO_ID+" selected=\"selected\">"+n.TYPETWO_NAME+"</option>";
					});
