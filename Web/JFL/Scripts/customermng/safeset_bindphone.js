...
(function($){
	$(function(){
		...
		/*-----页面节点获取-----*/
		/*step1*/
		var ss_emailpnsou=$("#ss_emailpnsou"),ss_emailpntar=$("#ss_emailpntar"),ss_emailgc=$("#ss_emailgc"),ss_emailtime=$("#ss_emailtime"),ss_validbp1=$("#ss_validbp1"),ss_emailpntartip=$("#ss_emailpntartip");
		/*step2*/
		var ss_validbp2=$("#ss_validbp2"),ss_emailge=$("#ss_emailge"),ss_emailtext21=$("#ss_emailtext21"),ss_emailtext21tip=$("#ss_emailtext21tip");
		...
		ss_emailtime.html("");
		/*发送短信验证step2*/
		ss_emailge.val("发送短信验证").removeClass("ss_emailungc").removeAttr("disabled");
		/*获取验证码*/
		ss_emailgc.click(function(){
			var pn_tar=ss_emailpnsou.val();
		...
		ss_emailge.click(function(){
			...
			if(ss_et2==""){
				ss_emailtext21tip.html("新手机不能为空");
				return false;
			}else if(!cregobj.Mobile.test(ss_et2)){
				ss_emailtext21tip.html("新手机格式不正确");
				return false;
			}else{
				/*倒计时操作*/
				...
				/*发送并获取验证码*/
				ss_emailtext21tip.html("正在获取验证码......");
				$.ajax({
					url:"/CustomerMng/SendSMS",
					type:"post",
					async:false,
					...
					success: function(res){
						if(res.State){
							...
							ss_emailtext21tip.html("获取验证码失败");
						}
					},
					error:function(e){
						ss_emailtext21tip.html("获取验证码失败");
		...
		function email_eTimes(o,c){
			...
			if(c==-1||c<0){
				...
				clearTimeout(mid1);
				mid1=null;
				o.val("发送短信验证").removeClass("ss_emailungc").removeAttr("disabled");
				return false;
			}
			c--;
		...
		var bp_vobj={
				...
				beforeSubmit:function(curform){
					...
					if(curid.indexOf("bp1")!=-1){
						/*
							to do
							修改邮箱第一步提交
						*/
					}else if(curid.indexOf("bp2")!=-1){
						/*
							to do
							修改邮箱密码第二步提交
						*/
		...
		/*表单校验*/
		/*修改密码1*/
		var ss_bpvobj1=ss_validbp1.Validform(bp_vobj);
		ss_bpvobj1.addRule(vmsg_objs().ss_bp1msg);
		/*修改密码2*/
		var ss_bpvobj2=ss_validbp2.Validform(bp_vobj);
		ss_bpvobj2.addRule(vmsg_objs().ss_bp2msg);
	});
})(jQuery);
