...
$(function() {
	tb = $('#div_table03').table({
		 		 ...
		 		 return [
					$.jsonToControl(
					 {tag:'a',style:'cursor: pointer', child:{tag:'span',child:'上架'},events:{click:function(){
						 $.post('picReturn/picReturnUL.do',{id:r.id,status:0},function(result){
						 		if(result==true){
					 ...
					 $.jsonToControl(
							 {tag:'a',style:'cursor: pointer', child:{tag:'span',child:'下架'},events:{click:function(){
								 $.post('picReturn/picReturnUL.do',{id:r.id,status:1},function(result){
								 		if(result==true){
				...
				$.jsonToControl({
					...
					events: {
						click: function() {
							...
							$("#windowIf").attr("src", 'picReturn/picReturnEdite.do?id=' + r.id);
							$("#windowDiv").window({
								title: '编辑首页图片广告',
								width: 650,
								height: 670
		...
		url: 'picReturn/picReturnList.do',
		autoload: true,
		total: 'totalCount',
		root: 'items',
		footbar: {
...
function query()
{
	tb.load({start:0,type:$('#type').val(),status:$('#status').val()});
}
...
function toAdd() {
	window.frames["windowIf"].document.body.innerHTML = "";
	document.getElementById("windowIf").style.display = "";
	$("#windowIf").attr("src", "picReturn/picReturnEdite.do");
	$("#windowDiv").window({
		title: '新增首页图片广告',
		width: 650,
		height: 670
