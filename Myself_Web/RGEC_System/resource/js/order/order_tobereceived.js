...
/*程序入口*/
require(['jquery','bootstrap','dialog','querydata','validform','common','submenu','themetitle','pagination','sort','checkbox','gridaction','date97','datepick','rule','commonfn'],function($,$strap,undefined,undefined,undefined,Common,undefined,undefined,undefined,Sort,CheckBox,GridAction,undefined,undefined,Rule,CommonFn) {
	$(function() {
			//页面元素引用
			var $submenu=$('#submenu'),
			...
			//html模板
			var htmlcontent='<tr><td><input value="$chkId" data-state="1" type="checkbox" name="chkId"></td><td><p>$companyNo</p></td><td><p>$libraryNo</p></td><td>$sendName</td><td>$recName</td><td>$orderStatus</td><td><em>$sendStatus</em></td><td><em>$recStatus</em></td><td>$dealer</td><td>$store</td><td><p>$total</p></td><td><p>$FV</p></td><td><span data-action="$actiontype" data-value="$actionvalue">操作</span></td></tr>';
			...
			//查询搜索
			$grid_querybtn.queryListData([$receiptno,$datetype,$datestart,$dateend,$agentno,$sendstatus,$recstatus],2000,function(sparam){
					...
					$.ajax({
							...
							success: function(data){
									...
									if(data){
										...
										$orderinfo_griddata.html('<tr><td colspan="13">暂无数据</td></tr>');
									}
							},
							error: function(){
									$orderinfo_griddata.html('<tr><td colspan="13">暂无数据</td></tr>');
