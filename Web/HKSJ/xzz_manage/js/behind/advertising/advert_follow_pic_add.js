...
function add()
{
	...
	{
		$.messager.alert("提示","价格不能为空","info",function(){document.getElementById("price").focus();});
		return;
	}
	if(!(/^[1-9][0-9]{0,9}$/.test($("#price").val()))){
		$.messager.alert("提示","价格最多为10位有效数字","info",function(){document.getElementById("price").focus();});
		return;
	}
	var imgUrl = $("#urlF").val();
	if(imgUrl.length>0){
	...
	$("#advertFollowForm").ajaxSubmit({
        type:'post',
        url:'advertising/advertFollowPicSave.do',
