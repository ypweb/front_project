...
;(function($, window, undefined){
	...
	{	
		...
		public_vars.$main_menu_wrap          = public_vars.$pageContainer.find('.sidebar-menu');
		public_vars.$mainmenu             = public_vars.$main_menu_wrap.find('.main-menu');
		
		public_vars.$horizontalNavbar     = public_vars.$body.find('.navbar.horizontal-menu');
		public_vars.$horizontalMenu       = public_vars.$horizontalNavbar.find('.navbar-nav');
		
		public_vars.$main_content          = public_vars.$pageContainer.find('.main-content');
		...
		public_vars.$page_loading_wrap   = public_vars.$body.find('.page-loading-overlay');
...
{
	if(public_vars.$main_menu_wrap.length)
	{
		var $items_with_subs = public_vars.$main_menu_wrap.find('li:has(> ul)'),
			toggle_others = public_vars.$main_menu_wrap.hasClass('toggle-others');
...
{
	if($li.data('is-busy') || ($li.parent('.main-menu').length && public_vars.$main_menu_wrap.hasClass('collapsed')))
		return;
	...
	var $sub_items 	  = $sub.children(),
		...
		current_y		 = public_vars.$main_menu_wrap.scrollTop(),
		item_max_y		= $li.position().top + current_y,
		fit_to_viewpport  = public_vars.$main_menu_wrap.hasClass('fit-in-viewport');
...
function setup_horizontal_menu()
{
	if(public_vars.$horizontalMenu.length)
	{
		...
		if(click_to_expand)
		{
			public_vars.$main_content.add( public_vars.$main_menu_wrap ).on('click', function(ev)
			{
...
{
	public_vars.$mainFooter.add( public_vars.$main_content ).add( public_vars.$main_menu_wrap ).attr('style', '');
...
function ps_update(destroy_init)
{
	...
	{
		if(public_vars.$main_menu_wrap.hasClass('collapsed'))
		{
			return;
		}
		
		public_vars.$main_menu_wrap.find('.sidebar-menu-inner').perfectScrollbar('update');
...
function ps_init()
{
	...
	if(jQuery.isFunction(jQuery.fn.perfectScrollbar))
	{
		if(public_vars.$main_menu_wrap.hasClass('collapsed') || ! public_vars.$main_menu_wrap.hasClass('fixed'))
		{
			return;
		}
		
		public_vars.$main_menu_wrap.find('.sidebar-menu-inner').perfectScrollbar({
			wheelSpeed: 2,
...
function ps_destroy()
{
	if(jQuery.isFunction(jQuery.fn.perfectScrollbar))
	{
		public_vars.$main_menu_wrap.find('.sidebar-menu-inner').perfectScrollbar('destroy');
	}
}
