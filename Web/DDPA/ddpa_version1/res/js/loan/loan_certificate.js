...
(function($){
	$(function(){
		...
		var cpar=ctar.parent(),cparp=cpar.parent(),cimg=ctar.find("li").eq(cindex).find("img"),ctitle=cimg.attr("alt"),cnums=ctar.find("li").size(),clen=ctar.width();
		var cobjs={
			"ctar":ctar,
			"cindex":cindex,
			"cpar":cpar,
...
*/
function lc_init(o){
	var co=o,ch=co.cimg.height();
	co.cpar.css({"height":ch});
	co.cparp.css({"height":ch});
	co.ctar.css({"height":ch,"left":-(640*co.cindex)});
	co.ctwrap.html(co.ctitle);
	co.cnwrap.html("("+parseInt(co.cindex+1,10)+"/"+co.cnums+")");
}
/*
播放事件
