...
require(['jquery','bootstrap','common','dialog','rule','validform','commonfn'], function($,$strap,undefined,undefined,Rule,undefined,CommonFn) {
	$(function() {
		/*页面元素引用*/
		var $fiAcInputForm=$('#fiAcInputForm'),
				$money=$('#money'),
		...
		$money.on('keyup',function(e){
				...
				$moneyup.text(CommonFn.toUpMoney(val[1]));
		});
		...
		$validobj=$fiAcInputForm.Validform({
				ajaxPost: true,
				datatype:{
					//自定义校验方式
				...
				beforeSubmit: function(curform) {
					...
					setTimeout(function(){
						...
						if(issucces){
							...
							window.location.href='recharge_manage.html#apply_recharge';
						}
					},3000);
					return false;
				},
