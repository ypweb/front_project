...
$(function() {
	tb = $('#div_table03').table({
		 		 ...
		 		 return [
					$.jsonToControl(
					 {tag:'a',style:'cursor: pointer', child:{tag:'span',child:'上架'},events:{click:function(){
						 $.post('advertising/advertFollowRecordUL.do',{id:r.id,status:0},function(result){
						 		if(result==true){
					 ...
					 $.jsonToControl(
							 {tag:'a',style:'cursor: pointer', child:{tag:'span',child:'下架'},events:{click:function(){
								 $.post('advertising/advertFollowRecordUL.do',{id:r.id,status:1},function(result){
								 		if(result==true){
				...
				$.jsonToControl({
					...
					events: {
						click: function() {
							...
							$("#windowIf").attr("src", 'advertising/advertFollowEdit.do?id=' + r.id);
							$("#windowDiv").window({
								title: '编辑随记广告',
								width: 500,
								height: 600
		...
		url: 'advertising/advertFollowList.do',
		autoload: true,
		total: 'totalCount',
		root: 'items',
		footbar: {
...
function query()
{
	tb.load({start:0,content:$('#content').val(),status:$('#status').val(),isDue:$("#isDue").val()});
}
...
function toAdd() {
	window.frames["windowIf"].document.body.innerHTML = "";
	document.getElementById("windowIf").style.display = "";
	$("#windowIf").attr("src", "advertising/advertFollowEdit.do");
	$("#windowDiv").window({
		title: '新增随记广告',
		width: 500,
		height: 600
