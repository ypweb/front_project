...
(function($){
	$(function(){
		/*数据全为空时的全局标识*/
		var isnodata=false,isnodata1=false,isnodata2=false,isnodata3=false,isnodata4=false;
		...
		var global_pie1=["利息","居间服务费","成交服务费","逾期还款违约金","提前还款违约金"],global_pie2=["本金","利息","居间服务费","成交服务费","逾期还款违约金","提前还款违约金"];
		/*初始化日期数据*/
		...
		$("#lr_pietable1").find("tr").each(function(index){
			...
			if(index==4&&lr_piesum1==0){
				pienodata1=true;
			}else{
				pienodata1=false;
			}
			lr_piedate1.push([global_pie1[index],curtext]);
			curtd.text(tradeRMBFormat("",ctext1));
		});
		...
		var lr_raobj={
			...
			title:{text:''},
			colors:["#fcdaa5","#fbb634","#7ecef4","#ec313a"],
			legend:{
				enabled:false,
				margin:35,
				padding:10,
			...
			tooltip:{
				shadow:false,
				formatter: function(){
					return "<span>"+this.x+"</span><br /><span style=\"color:"+this.series.color+";\">"+this.series.name+":</span><span style=\"font-weight:bold;\">"+tradeRMBFormat("",this.y)+"</span>"
				}
			},
			plotOptions: {
				series: {
		...
		var lr_ccdobj={
			...
			colors:["#fbb634","#7ecef4","#ccd2ee","#ec313a","#ec8372"],
			credits: {enabled:false},
			series: [{
					name:["累积成本明细百分比"],
					data:lr_piedate1
		...
		var lr_lraobj={
			...
			colors:["#fcdaa5","#fbb634","#7ecef4","#ccd2ee","#ec313a","#ec8372"],
			credits: {enabled:false},
			series: [{
					name:["已还明细百分比"],
					data:lr_piedate2
		...
		curChart = $('#lr_radatachart').highcharts();
		//数据过滤
		$("#switch").click(function(e){
			var $target = $(e.target), dType = $target.attr("data-type");
			$target.removeClass("cur"); 
			$target.siblings("a").addClass("cur");
			switch(dType){
				case "bj": 
					curChart.series[0].show();
					curChart.series[1].hide();
					curChart.series[2].hide();
					curChart.series[3].hide();
					break;
				case "lx":
					curChart.series[0].hide();
					curChart.series[1].show();
					curChart.series[2].hide();
					curChart.series[3].hide();
					break;
				case "fee":
			...
			$.ajax({
				...
				error:function(){
					...
					if(r==0){
						...
						isnodata=false;
						handleDatas(undefined,ctimes,true);
					}
				}
			});
		}
		...
		/*数据处理*/
		function handleDatas(objs,ctimes,str){
			if(str){
					var tt=0;
					for(tt;tt<ctimes;tt++){
							var ttr1=parseInt(Math.random()*10000),
									ttr2=parseInt(Math.random()*10000),
									ttr3=parseInt(Math.random()*10000),
									ttr4=parseInt(Math.random()*10000);
									
									lr_raresult1.push(ttr1);
									lr_raresult2.push(ttr2);
									lr_raresult3.push(ttr3);
									lr_raresult4.push(ttr4);
									lr_sum+=(function(){return ttr1+ttr2+ttr3+ttr4})(ttr1,ttr2,ttr3,ttr4);
					}
					return;
			}
		...
		/*地图功能*/
		/*初始化数据*/
		var dataColorObj={0:{colors:"#f4e3d5"},1:{colors:"#fcdaa5"},2:{colors:"#fcae91"},3:{colors:"#fb6a4a"},4:{colors:"#cb181d"},5:{colors:"#c51016"}};
...
function lr_Top(counts,objs){
	...
	for(i=0;i<topnum;i++){
		...
		if(i==0){
			document.getElementById("wr_tzfb").innerHTML=tempres[1]+"人";
		}
