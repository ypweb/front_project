var select; //选择行数据	
		...
		cm:[
					...
					,{ 	dataindex:'verno', // 显示数据在行中的属性
					 	headtext:'版本号'
					}
					,{ 	dataindex:'os', // 显示数据在行中的属性
					 	headtext:'客户端类型',
					...
					,{ 	dataindex:'isForceUpdate', // 显示数据在行中的属性
					 	headtext:'是否强制更新',
					 	render : function(i, j, c, r) {
							if('0' == c){
								return '不强制更新';
					 		 ...
					 		 return [
					 		       $.jsonToControl(
									 {tag:'a',style:'cursor: pointer', child:{tag:'span',child:'编辑'},events:{click:function(){
											...
											$("#windowIf").attr("src",'/xzzBehind/phoneversion/toEdit.do?id='+r.id);
											$("#windowDiv").window({title : '编辑版本:'+r.verno,width:470,height:480}).window('open');
										 return false;}}})
								];
					 		}
			...
			,
			url:'/xzzBehind/phoneversion/toQuery.do' ,
			autoload:true,
			total:'totalCount',
			root:'items'
			,
	...
	function toAdd()
	{
		window.frames["windowIf"].document.body.innerHTML = "";
		document.getElementById("windowIf").style.display="";
		$("#windowIf").attr("src","/xzzBehind/phoneversion/toAdd.do");
		$("#windowDiv").window({title : '新增版本更新',width:450,height:450}).window('open'); 
	}
