...
function templateShow(rid,rdata,rwrap,rway){
	...
	if(rway=="add"||rway=="formadd"){
		...
		if(reslen>=1||reslen=="form"){
			...
			}else if(tempid=="world"){
				curtr.eq(1).html("<span class=\"dataimgnone\">reslen+"+reslen+"个</span>");
			}
			if(rway=="add"){
		...
		if(reslen!=0){
			...
			if(tempid!="world"){
				...
				curtr.eq(1).html("<span class=\"dataimgnone\">0个</span>");
			}
			curtr.eq(4).html("");
			curtr.last().find("input.delete").removeAttr("id").addClass("deletenone");
		}
...
function showSlider(ids,aindex,vstate){
	if(ids=="world"||ids=="video"){
		...
		for(var i=0;i<imglen;i++){
			...
			imgstr+="<li><div><img style=\"width:150px;height:100px;overflow:hidden;display:block;\" alt=\"\" src=\""+timgurl[0]+timgurl[1]+"\"/><span></span></div></li>";
		}
...
function fbox_Init(){
		...
		$("#oimgtomin,#oimgtomax,#oimgnormal,#oimgrotate,#oimgdelete").live("click",function(e){
				...
				if(curid=="oimgtomin"||curid=="oimgtomax"||curid=="oimgnormal"){
					...
					for(var i=0;i<imglen;i++){
						var timgurl=imageTypeAdapt(imgobj[i]);
						imgstr+="<li><div><img alt=\"\" style=\"width:150px;height:100px;overflow:hidden;display:block;\" src=\""+timgurl[0]+timgurl[1]+"\"/><span></span></div></li>";
					}
					var param={},tarwrap=1;
...
function handleForms(cid){
	...
	if(curid=="education"){
		...
		$.ajax({
			type:'post',
			url:'addCertificateForEducationNo',
			async:false,
			data : params,
			dataType : 'json',
			success : function(data){
...
function showSlideImage(imgdata){
	...
	for(var m=0;m<datalen;m++){
		...
		tagstr+="<a class=\"grouped_elements\" target=\"_blank\" title=\"图片预览："+imgcur+"/"+datalen+"\" rel=\"group\" href=\""+timgurl[0]+"\"><img style=\"width:130px;height:80px;overflow:hidden;display:block;\" src=\""+timgurl[0]+timgurl[1]+"\" alt=\"\"/></a>";
	}
...
function viewSee(iobj,inode){
	...
	var tagstr="",imgcur=0,dataobj=execres[tempa],datalen=dataobj.length,timgurl=[],m=0;
	for(;m<datalen;m++){
		imgcur=parseInt(m+1,10);
		timgurl=imageTypeAdapt(dataobj[m]);
		tagstr+="<a class=\"grouped_elements\" target=\"_blank\" title=\"图片预览："+imgcur+"/"+datalen+"\" rel=\"group\" href=\""+timgurl[0]+"\"><img style=\"width:130px;height:80px;overflow:hidden;display:block;\" src=\""+timgurl[0]+timgurl[1]+"\" alt=\"\"/></a>";
	}
...
var isopened=false;
(function(){
	var di_open=$("#datainfo_popopen"),di_close=$("#datainfo_popclose"),di_wrap=$("#datainfo_popwrap");
	di_open.click(function(e){
		if(isopened){
			di_wrap.hide();
			isopened=false;
		}else{
