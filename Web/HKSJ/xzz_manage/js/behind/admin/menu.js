var select; //选择行数据	
		...
		cm:[
					...
					,{ 	dataindex:'menuName', // 显示数据在行中的属性
					 	headtext:'菜单名称'
					}
					,{ 	dataindex:'url', // 显示数据在行中的属性
					 	headtext:'菜单url'
					}
					,{ 	dataindex:'parentMenuName', // 显示数据在行中的属性
					 	headtext:'上级菜单'
					}
					,{ 	dataindex:'nortno', // 显示数据在行中的属性
					 	headtext:'序列号'
					}
					 		 ...
					 		 return [
					 		       $.jsonToControl(
									 {tag:'a',style:'cursor: pointer', child:{tag:'span',child:'编辑'},events:{click:function(){
											...
											$("#windowIf").attr("src",'toUpdateMenu.do?id='+r.id);
											$("#windowDiv").window({title : '编辑菜单:'+r.menuName,width:400,height:400}).window('open');
										 return false;}}})
							] ;
						}
					}
	...
	function query()
	{
		tb.load({start:0,menuName:$('#menuName').val(),descc:$('#descc').val()});
	}
	...
	function toAdd()
	{
		...
		$("#windowDiv").window({title : '新增角色',width:400,height:400}).window('open'); 
	}
