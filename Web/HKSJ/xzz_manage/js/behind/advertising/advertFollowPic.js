...
$(function() {
	tb = $('#div_table03').table({
		 		 ...
		 		 return [
					$.jsonToControl(
					 {tag:'a',style:'cursor: pointer', child:{tag:'span',child:'上架'},events:{click:function(){
						 $.post('advertising/advertFollowUL.do',{id:r.id,status:0},function(result){
						 		if(result==true){
					 ...
					 $.jsonToControl(
							 {tag:'a',style:'cursor: pointer', child:{tag:'span',child:'下架'},events:{click:function(){
								 $.post('advertising/advertFollowUL.do',{id:r.id,status:1},function(result){
								 		if(result==true){
				...
				$.jsonToControl({
					...
					events: {
						click: function() {
							...
							$("#windowIf").attr("src", 'advertising/advertFollowPicEdit.do?id=' + r.id);
							$("#windowDiv").window({
								title: '编辑随记图片广告',
								width: 500,
								height: 500
		...
		url: 'advertising/advertFollowPicList.do',
		autoload: true,
		total: 'totalCount',
		root: 'items',
		footbar: {
...
function toAdd() {
	window.frames["windowIf"].document.body.innerHTML = "";
	document.getElementById("windowIf").style.display = "";
	$("#windowIf").attr("src", "advertising/advertFollowPicEdit.do");
	$("#windowDiv").window({
		title: '新增随记图片广告',
		width: 500,
		height: 500
