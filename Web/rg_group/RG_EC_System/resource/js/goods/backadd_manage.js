...
require(['jquery','bootstrap','common','dialog','querydata','sort','checkbox'], function($,$strap,undefined,undefined,undefined,Sort,CheckBox) {
	$(function() {
		/*页面元素引用*/
		var $backadd_griddata=$('#backadd_griddata'),
				$orderNo=$('#orderNo'),
				$grid_querybtn=$('#grid_querybtn'),
				$backadd_sort=$('#backadd_sort'),
				...
				$.ajax({
						...
						success: function(data){
								...
								if(data){
										...
										$backadd_griddata.html(temphtml.join(''));
								}else{
									$backadd_griddata.html('<tr><td colspan="7">暂无数据</td></tr>');
								}
						},
						error: function(){
								$backadd_griddata.html('<tr><td colspan="7">暂无数据</td></tr>');
		...
		//绑定排序切换
		$backadd_sort.delegate('th.issort', 'click', function () {
		...
		//全选与取消全选
		//初始化
		CheckBox.init($backadd_griddata,$chkId_selector);
		//绑定全选与取消全选
		$chkId_selector.click(function(){
			CheckBox.toggleCheckAll($chkId_selector, $backadd_griddata);
			//如果需要获取详细已选择数据值：可调用CheckBox.getCheckBox()方法
		});
		//绑定单个选中与取消
		$backadd_griddata.delegate('td>input:checkbox', 'click', function () {
		...
		//绑定退待发货、换待发货操作事件
		$.each([$returnback,$changeback],function(i,v){
				var $this=this,
				selector=$this.selector.slice(1),
				dia=dialog(),
				txt='';
				this.on('click',function(){
						var backid=CheckBox.getCheckBox();
						if(backid.length==0){
						...
						if(selector=='returnback'){
								//to do
								//退待发货业务逻辑
								console.log('退待发货:'+params);
						}else if(selector=='changeback'){
								//to do
								//换待发货业务逻辑
								console.log('换待发货:'+params);
						}
				})
		});
