/*全局变量*/
var execmap={"card":0,"work":1,"income":2,"credit":3,"house":4,"driving":5,"marry":6,"education":7,"title":8,"video":9,"phone":10,"live":11,"world":12,"other":13,"organization":14,"tax":15};
var execres=[[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[]];
...
(function($){
	$(function(){
		...
		/*证件添加*/
		$("#cardadd,#workadd,#incomeadd,#creditadd,#organizationadd,#taxadd,#houseadd,#drivingadd,#marryadd,#educationadd,#titleadd,#videoadd,#phoneadd,#liveadd,#worldadd,#otheradd").live("click",function(){
			...
			if(curid=="education"){
				...
				$.post("queryCertificateForEducationNo","",function(data){
					var edudata=data.educationNo;
					if(data.hasOwnProperty("educationNo")||edudata!="undefined"){
						if(/\d{12}/.test(edudata)){
							document.getElementById("education_online").value=edudata;
							/*
							to do
							调用第三方数据查询
							*/
				...
				$.ajax({
					...
					data:{
						"certificateTypeId":parseInt(tempa+15,10),
						"loanId":$("#loanId").val()
					},
					dataType:"json",
			...
			$.ajax({
				...
				data:{"certificateTypeId":parseInt(tempa+15,10),"loanId":$("#loanId").val(),rnd:Math.random()},
				dataType:"json",
				success: function(datas){
		...
		/*证件删除*/
		var isdelete=false;
		$("#carddelete,#workdelete,#incomedelete,#creditdelete,#organizationdelete,#taxdelete,#housedelete,#drivingdelete,#marrydelete,#educationdelete,#titledelete,#videodelete,#phonedelete,#livedelete,#worlddelete,#otherdelete").live("click",function(){
				...
				if(curid!="world"){
					...
					if(execres[tempa].length==0||typeof tempa=="undefined"||tempa==""){
						/*alert("none datas");*/
						var tdatas={"certificateTypeId":parseInt(tempa+15,10),"loanId":$("#loanId").val()};
						/*alert("params: "+tdatas.certificateTypeId+"====="+tdatas.loanId);*/
						$.ajax({
							async:false,
							url:"queryCertificateImages",
							data:tdatas,
				...
				param["certificateTypeId"]=Number(execmap[curid])+15;
				$.post("deleteCertificate",param,function(data){
					if(data.success){
						...
						infoTipAlert("删除成功","succ",delsucc);
					}else{
		...
		/*执行上传*/
        $("#card_action,#work_action,#income_action,#credit_action,#organization_action,#tax_action,#house_action,#driving_action,#marry_action,#education_action,#title_action,#video_action,#phone_action,#live_action,#world_action,#other_action").click(function(){
			...
			var param={};
			param["certificateTypeId"]=Number(tempa)+15;
			param["loanId"]=$("#loanId").val();
...
KindEditor.ready(function(K){
	var editor = K.editor({
		uploadJson : '../../upload/kindetor/imageUpload?thumbSizes=150x100&imgSize=8',		
		imageSizeLimit : "8MB",
		allowFileManager : true
	});
	K("#card_up,#work_up,#income_up,#credit_up,#organization_up,#tax_up,#house_up,#driving_up,#marry_up,#education_up,#title_up,#phone_up,#live_up,#world_up,#other_up").click(function(){
...
/*kindeditor load 批量上传*/
KindEditor.ready(function(K){
	var editor = K.editor({
		uploadJson : '../../upload/kindetor/multiimageUpload?thumbSizes=150x100&imgSize=8&userId='+$("#userId").val(),		
		imageSizeLimit : "8MB",
		allowFileManager : true
	});
	...
	/*批量上传*/
	K("#card_batchup,#work_batchup,#income_batchup,#credit_batchup,#organization_batchup,#tax_batchup,#house_batchup,#driving_batchup,#marry_batchup,#education_batchup,#title_batchup,#phone_batchup,#live_batchup,#world_batchup,#other_batchup").click(function(){
...
KindEditor.ready(function(K){
	var editor = K.editor({
		uploadJson : '../../upload/kindetor/videoUpload?imgSize=100',		
		imageSizeLimit : "20MB",
		allowFileManager : true
	});
	K("#video_up").click(function(){
		var curobj=K(this),curid=curobj.attr("id").slice(0,-3);
		/*执行文件上传*/
		editor.loadPlugin('imagevideo', function() {
			editor.plugin.imageDialog({
				showRemote : false,
				imageUrl:K("#"+curid+"_url").val(),
				clickFn : function(url, title, width, height, border, align) {
...
/*删除slide预览*/
$("#card_slide,#work_slide,#income_slide,#credit_slide,#organization_slide,#tax_slide,#house_slide,#driving_slide,#marry_slide,#education_slide,#title_slide,#video_slide,#phone_slide,#live_slide,#world_slide,#other_slide").live("click",function(e){
